...
void sig_Handler(int sinal)
{
    ...
    if (systemTime - lastQuantum < quantum)
    {
        return;
    } // early return para quando a tarefa estiver dentro do quantum
...
void before_task_switch(task_t *task)
{
    ...
    task->activations++;
    if (task == taskMain && systime() > 0 && countTasks == 1)
    { // processos já terminaram e pode imprimir a contabilização da main
        printContab(taskExec);
    }
}
